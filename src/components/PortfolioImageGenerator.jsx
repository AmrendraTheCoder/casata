...
const PortfolioImageGenerator = ({ stats, healthScore, address }) => {
  ...
  // Generate random card attributes based on health score
  ...
  const downloadImage = async () => {
    ...
    try {
      ...
      link.download = `lampfi-prosperity-card-${cardAttributes.uniqueId}-${Date.now()}.png`;
      link.href = canvas.toDataURL('image/png');
      link.click();
    } catch (error) {
      console.error('Error generating prosperity image:', error);
    } finally {
  ...
  return (
    <div className="card p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-xl font-bold bg-gradient-to-r from-lamp-gold-400 to-lamp-gold-500 bg-clip-text text-transparent">🪔 Prosperity Card Generator</h3>
        ...
        >
          {isGenerating ? '⏳ Summoning...' : '✨ Generate Prosperity Image'}
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <div className="bg-gradient-to-br from-lamp-purple-900/40 to-lamp-magenta-900/40 border border-lamp-purple-500/30 rounded-lg p-3">
          ...
          <div className="text-lg font-bold text-lamp-purple-200">{cardAttributes.rarity}</div>
        </div>
        <div className="bg-gradient-to-br from-lamp-magenta-900/40 to-lamp-purple-900/40 border border-lamp-magenta-500/30 rounded-lg p-3">
          ...
          <div className="text-lg font-bold text-lamp-magenta-300">{cardAttributes.edition}</div>
        </div>
        <div className="bg-gradient-to-br from-lamp-teal-900/40 to-lamp-teal-800/40 border border-lamp-teal-500/30 rounded-lg p-3">
          ...
          <div className="text-lg font-bold text-lamp-teal-300 capitalize">{cardAttributes.pattern}</div>
        </div>
        <div className="bg-gradient-to-br from-lamp-gold-900/40 to-lamp-gold-800/40 border border-lamp-gold-500/30 rounded-lg p-3">
          ...
          <div className="text-lg font-bold text-lamp-gold-300 font-mono">{cardAttributes.uniqueId}</div>
        </div>
      </div>
      ...
      <div className="border-4 border-lamp-purple-500/30 rounded-2xl p-6 bg-gradient-to-br from-lamp-night-800 to-lamp-night-900">
        ...
        <div
          ref={imageRef}
          className={`relative bg-gradient-to-br ${cardAttributes.palette.bg} rounded-3xl p-8 text-white overflow-hidden`}
          style={{ width: '600px', maxWidth: '100%', margin: '0 auto', aspectRatio: '1/1' }}
        >
          {/* Background Pattern */}
          <BackgroundPattern pattern={cardAttributes.pattern} />
          ...
          <div className="relative z-10">
            ...
            <div className="text-center mb-6">
              ...
              <div className="text-6xl mb-3">{cardAttributes.badge}</div>
              <h1 className={`text-4xl font-black mb-2 bg-gradient-to-r ${cardAttributes.palette.accent} bg-clip-text text-transparent`}>
                {cardAttributes.rarity.toUpperCase()}
              </h1>
              ...
              <div className="relative bg-white/10 backdrop-blur-md rounded-2xl p-6 border-2 border-white/30">
                <div className="text-center">
                  ...
                  <div className="flex items-center justify-center gap-2">
                    <div className={`text-7xl font-black bg-gradient-to-br ${cardAttributes.palette.accent} bg-clip-text text-transparent`}>
                      {safeHealthScore}
                    </div>
                  ...
                  <div className="w-full bg-white/20 rounded-full h-3 mt-4 overflow-hidden">
                    <div
                      className={`h-3 rounded-full bg-gradient-to-r ${cardAttributes.palette.accent} transition-all duration-1000`}
            ...
            <div className="flex items-center justify-between pt-4 border-t border-white/20">
              <div>
                ...
                <div className="text-sm font-bold">{cardAttributes.edition}</div>
              </div>
              <div className="text-center">
                <div className="text-xl font-black">LAMPFI</div>
                <div className="text-xs text-white/70">Prosperity Card</div>
              </div>
              <div className="text-right">
                <div className="text-xs text-white/60">ID</div>
                <div className="text-sm font-bold font-mono">{cardAttributes.uniqueId}</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-lamp-purple-200">
                Each prosperity score generates a unique card with mystical colors, patterns, and rarity based on your lamp's activity!
              </p>
            </div>
          </div>
        </div>
        <div className="p-4 bg-gradient-to-br from-lamp-teal-900/40 to-lamp-teal-800/40 border border-lamp-teal-500/30 rounded-xl">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-lamp-purple-200">
                Download your prosperity card and share it on Twitter, Discord, Telegram, or use it as your profile picture to show off your mystical prosperity!
              </p>
            </div>
          </div>
        </div>
